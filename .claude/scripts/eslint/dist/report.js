 ...
 */
import { createESLint, categorizeByRisk, parseArgs, getRiskEmoji, formatFilePath, estimateFixTime, getRelativePath, } from './helpers.js';
async function generateReport(targetPath, detailed = false) {
    ...
    if (analysis.summary.medium > 0) {
        ...
        if (detailed) {
            const mediumRiskByFile = new Map();
...
async function main() {
    ...
    const targetPath = args['path'] || 'resources/js';
