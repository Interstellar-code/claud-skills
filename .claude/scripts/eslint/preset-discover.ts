 ...
 */
async function discoverFromPreset(presetName: string): Promise<{
    feature: string;
    description: string;
    files: string[];
}> {
 ...
 */
function formatResult(result: { feature: string; description: string; files: string[] }, detailed: boolean = false): void {
    console.log('\n' + '='.repeat(80));
...
async function main() {
    ...
    try {
        const result = await discoverFromPreset(presetName as string);
