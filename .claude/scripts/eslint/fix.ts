...
async function fixESLint(options: FixOptions): Promise<void> {
    ...
    const results = (await eslintAnalyze.lintFiles([targetPath])) as FileIssues[];
    ...
    const filesToWrite = fixResults.filter((result: any) => {
        const hasFixableIssues = result.messages.some((msg: any) =>
            issuesToFix.some(
                (issue) => issue.filePath === result.filePath && issue.ruleId === msg.ruleId
            )
        );
    ...
    if (dryRun) {
        ...
        filesToWrite.forEach((result: any) => {
    ...
    await ESLint.outputFixes(filesToWrite as any);
...
function filterIssuesByRisk(
    analysis: ReturnType<typeof categorizeByRisk>,
    maxRisk: RiskLevel
): ReturnType<typeof categorizeByRisk>['byRisk']['LOW'] {
    const issues = [];
...
async function main() {
    ...
    const options: FixOptions = {
        risk: (args['risk'] as RiskLevel) || 'LOW',
        dryRun: Boolean(args['dry-run']),
        force: Boolean(args['force']),
        targetPath: (args['path'] as string) || 'resources/js',
    };
