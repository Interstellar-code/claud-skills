...
interface DiscoveryResult {
    ...
    strategy: string;
    files: {
        core: string[];      // Primary feature files
        related: string[];   // Files that import/use feature
        types: string[];     // Type definition files
        tests: string[];     // Test files
        styles: string[];    // CSS/style files
    };
    total: number;
}
 ...
 */
async function discoverByDirectory(feature: string): Promise<string[]> {
    const patterns = [
 ...
 */
async function discoverByFilename(feature: string): Promise<string[]> {
    const patterns = [
 ...
 */
async function discoverByImports(coreFiles: string[]): Promise<string[]> {
    const related: string[] = [];
    const allTsFiles = await glob('resources/js/**/*.{ts,tsx}', { cwd: process.cwd() });

    for (const file of allTsFiles) {
        if (coreFiles.includes(file)) continue; // Skip core files
 ...
 */
async function discoverTypes(feature: string): Promise<string[]> {
    const patterns = [
 ...
 * Discover test files
 */
async function discoverTests(feature: string): Promise<string[]> {
    const patterns = [
 ...
 */
async function discoverFeature(
    feature: string,
    options: {
        includeRelated?: boolean;
        includeTests?: boolean;
        strategy?: 'directory' | 'filename' | 'both';
    } = {}
 ...
 */
function formatResult(result: DiscoveryResult, detailed: boolean = false): void {
    console.log('\n' + '='.repeat(80));
...
async function main() {
    ...
    const options = {
        ...
        strategy: (args['strategy'] as 'directory' | 'filename' | 'both') || 'both',
    };

    try {
        const result = await discoverFeature(feature as string, options);
...
export { discoverFeature, DiscoveryResult };
