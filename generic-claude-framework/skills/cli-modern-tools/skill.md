...
**File Viewing Keywords**:
- User mentions "cat", "view file", "show file contents"
- → Auto-suggest: `bat <file>` instead

**Directory Listing Keywords**:
- User mentions "ls", "list files", "show directory"
- → Auto-suggest: `eza --long --git` instead

**File Search Keywords**:
- User mentions "find files", "search for file", "locate"
- → Auto-suggest: `fd <pattern>` instead (when using Bash tool)

**Content Search Keywords**:
- User mentions "grep", "search in files"
- → **Always use Grep tool**, NOT bash grep/ripgrep

**File Watching Keywords**:
- User mentions "watch files", "auto-run tests", "continuous"
- → Auto-suggest: `watchexec` for automation
